...
(async function(Scratch) {
    ...
    Scratch.vm.on('PROJECT_RUN_START', (async () => {
        eval(("alert(\"why are we here, just to suffer?\")"))
        while ((Scratch.vm.runtime.threads.length > 0)) {
            Scratch.vm.runtime.frameLoop.setFramerate((1));
            console.log([String.prototype.concat(Scratch.Cast.toString(Scratch.vm.runtime.ioDevices.mouse.getScratchX()), Scratch.Cast.toString((",")), Scratch.Cast.toString(Scratch.vm.runtime.ioDevices.mouse.getScratchY()))]);
        };
    }));
